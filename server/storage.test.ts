...
describe("copyBudgets", () => {
  it("copies budgets from one month to another", async () => {
    const cat = await store.createCategory({ name: "Alimentação", icon: "circle", color: "orange", type: "despesa" });
  ...
  it("skips categories already budgeted in the target month", async () => {
    const cat = await store.createCategory({ name: "Transporte", icon: "circle", color: "slate", type: "despesa" });
    await store.createBudget({ categoryId: cat.id, limit: 200, month: "2026-03" });
    await store.createBudget({ categoryId: cat.id, limit: 300, month: "2026-04" }); // already exists
    ...
    const april = await store.getBudgets("2026-04");
    // The existing budget (300) should not be overwritten
...
describe("getMonthlySummary", () => {
  it("returns N months and sums receitas/despesas correctly", async () => {
    ...
    await store.createTransaction({ description: "Salário", amount: 3000, type: "receita", date: `${ym}-01` });
    await store.createTransaction({ description: "Aluguel", amount: 1200, type: "despesa", date: `${ym}-05` });
    await store.createTransaction({ description: "Mercado", amount: 400, type: "despesa", date: `${ym}-10` });
